...
import type { IndicatorStats } from "@/lib/stats"

type AreaTrend = "up" | "down"
...
type AreaDashboardClientProps = {
  areaParam: string
  alertsStats: IndicatorStats
  effStats: IndicatorStats
}

export function AreaDashboardClient({ areaParam, alertsStats, effStats }: AreaDashboardClientProps) {
  const normalizedKey = areaParam?.toLowerCase?.() ?? "seguranca"
  const area = areaData[normalizedKey] ?? areaData.seguranca
  const areaSlug = areaParam || "seguranca"
  const Icon = area.icon

  const computedKpis: AreaKpi[] = [
    {
      label: "Alertas Ativos",
      value: String(alertsStats.latest ?? area.kpis[2]?.value ?? "0"),
      change:
        alertsStats.change !== undefined
          ? `${(alertsStats.change * 100).toFixed(1)}%`
          : area.kpis[2]?.change ?? "0%",
      trend: alertsStats.change && alertsStats.change > 0 ? "up" : "down",
    },
    {
      label: "Eficiência",
      value: `${Math.round((effStats.latest ?? 0.87) * 100)}%`,
      change:
        effStats.change !== undefined
          ? `${(effStats.change * 100).toFixed(1)}%`
          : area.kpis[3]?.change ?? "0%",
      trend: effStats.change && effStats.change > 0 ? "up" : "down",
    },
    ...area.kpis.filter((_, index) => index < 2),
  ]
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Link href={`/areas/${areaSlug}/config`}>
